...
# Where to find the CA that signed the :8443 listener's certificate.
#
# Explicitly set: use it, and fail if it is not there. Someone who names a path
# is telling us where their trust anchor lives; quietly fetching a different one
# over the network would be the wrong answer to a typo.
#
# Not set: on the gateway host the file is already in place. From a client
# machine it is not, and falling back to $CA_CHAIN cannot work, the ML-DSA chain
# will never verify a classical ECDSA certificate, so the connection fails with
# no HTTP response at all. Fetch it from the bootstrap listener, which serves it
# unauthenticated for exactly this case. Step 3b then verifies the ISSUED
# certificate against $CA_CHAIN, so a substituted enrollment CA cannot get a
# forged certificate past this script.
if [[ -n "${PQC_ENROLL_CA:-}" ]]; then
    ENROLL_CA="$PQC_ENROLL_CA"
    if [[ ! -f "$ENROLL_CA" ]]; then
        echo "ERROR: PQC_ENROLL_CA is set to '$ENROLL_CA', which does not exist." >&2
        echo "  Point it at the enrollment CA, or unset it to fetch it from the" >&2
        echo "  gateway's :8443 listener automatically." >&2
        exit 1
    fi
else
    ENROLL_CA="/etc/pki/pqc-ca/enroll-classical-ca.crt"
    if [[ ! -f "$ENROLL_CA" ]]; then
        _GW_HOST_EARLY=$(echo "$GW" | sed 's|https://||; s|[:/].*||')
        ENROLL_CA="$(mktemp -t enroll-ca.XXXXXX)"
        if ! curl -fsSk --max-time 15 --connect-timeout 8 \
             "https://${_GW_HOST_EARLY}:8443/enroll-ca.crt" -o "$ENROLL_CA" 2>/dev/null \
           || ! grep -q "BEGIN CERTIFICATE" "$ENROLL_CA"; then
            rm -f "$ENROLL_CA"
            echo "ERROR: could not fetch the enrollment CA from ${_GW_HOST_EARLY}:8443" >&2
            echo "  Copy it from the gateway host and point PQC_ENROLL_CA at it:" >&2
            echo "    scp root@${_GW_HOST_EARLY}:/etc/pki/pqc-ca/enroll-classical-ca.crt ./enroll-ca.crt" >&2
            echo "    PQC_ENROLL_CA=./enroll-ca.crt $0 $*" >&2
            exit 1
        fi
        trap 'rm -f "$ENROLL_CA"' EXIT
        echo "    Fetched the enrollment CA from ${_GW_HOST_EARLY}:8443"
        echo "      SHA-256: $("${OSSL:-openssl}" x509 -in "$ENROLL_CA" -noout -fingerprint -sha256 2>/dev/null | sed 's/.*=//')"
        echo "      Compare it out of band if this deployment is not yours."
    fi
fi
...
if [[ -z "$CN" ]]; then
    ...
    echo "  admin.crt      scp root@<server>:<repo>/admin-cert/gateway-admin.crt ./admin.crt"
    echo "  admin.key      scp root@<server>:<repo>/admin-cert/gateway-admin.key ./admin.key && chmod 600 admin.key"
    echo "  (macOS/Docker fallback pulls the PQ-OpenSSL base image; it is public, no login needed)"
    exit 1
fi
...
if [[ "${_PQC_IN_DOCKER:-0}" != "1" ]] && ! has_pq_openssl; then
    ...
    if ! command -v docker &>/dev/null; then
        ...
        echo "  or let the Docker fallback pull the PQ-OpenSSL base image (public, no login)"
    ...
    #
    # The image is a public package; no registry login is required to pull it.
    PQC_BASE_IMAGE="${PQC_BASE_IMAGE:-ghcr.io/pqc-ra/pqc-mtls-openssl-base@sha256:700d3dba69e22fb604b9254b0ac4d2093e828d466507fb5255a8589480775173}"
...
# ── Step 3b: verify the issued certificate against the trust anchor ──────────
# The enrollment channel is authenticated by a CA this client may have fetched
# over that same channel. What makes that safe is this check: the issued
# certificate must chain to $CA_CHAIN, which arrives out of band. A substituted
# enrollment CA can serve a different bootstrap endpoint, but it cannot make the
# real ML-DSA intermediate sign a certificate, so a forged one fails here.
if ! "$OSSL" verify -CAfile "$CA_CHAIN" "$CRT_FILE" >/dev/null 2>&1; then
    echo "ERROR: the issued certificate does not chain to $CA_CHAIN" >&2
    "$OSSL" verify -CAfile "$CA_CHAIN" "$CRT_FILE" >&2 || true
    echo "  The enrollment endpoint returned a certificate this client cannot" >&2
    echo "  verify. Do not use it. Check that PQC_CA_CHAIN is this deployment's" >&2
    echo "  CA chain, and that :8443 is the gateway you intend to enroll with." >&2
    rm -f "$CRT_FILE"
    exit 1
fi
echo "    Verified against $CA_CHAIN"
...
if ! has_pq_curl; then
    ...
    _IDENT="-cert $_DISPLAY_OUT/$CN.crt -key $_DISPLAY_OUT/$CN.key"
    echo "  # Verify PQC handshake (expect: mldsa65 and X25519MLKEM768):"
    echo "  echo | OPENSSL_CONF=/etc/ssl/openssl.cnf $_OSSL_CMD s_client -connect ${_GW_HOST}:443 -CAfile $_DISPLAY_CA $_IDENT -tls1_3 2>&1 | grep -E 'Negotiated|Peer signature'"
    echo ""
    echo "  # Full API request (no output? drop the 2>/dev/null to see why):"
    echo "  printf 'GET /api/v1/status HTTP/1.1\\r\\nHost: ${_GW_HOST}\\r\\nConnection: close\\r\\n\\r\\n' | OPENSSL_CONF=/etc/ssl/openssl.cnf $_OSSL_CMD s_client -connect ${_GW_HOST}:443 -CAfile $_DISPLAY_CA $_IDENT -tls1_3 -quiet 2>/dev/null"
else
    _IDENT="-cert $_DISPLAY_OUT/$CN.crt -key $_DISPLAY_OUT/$CN.key"
    echo "  # Verify PQC handshake (expect: mldsa65 and X25519MLKEM768):"
    echo "  echo | OPENSSL_CONF=/etc/ssl/openssl.cnf /opt/openssl/bin/openssl s_client -connect ${_GW_HOST}:443 -CAfile $_DISPLAY_CA $_IDENT -tls1_3 2>&1 | grep -E 'Negotiated|Peer signature'"
    echo ""
    echo "  # Full API request (expect a JSON body from the backend):"
