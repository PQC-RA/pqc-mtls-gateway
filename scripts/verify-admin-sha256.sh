#!/usr/bin/env bash
# =============================================================================
# verify-admin-sha256.sh: Automated verification of the admin-authorization fix
# =============================================================================
# Proves, end-to-end from a real certificate and with NO project dependencies
# (only openssl + node), that:
#
#   1. The fingerprint the gateway derives, SHA-256(DER), is identical to the
#      value an operator pastes from `openssl x509 -fingerprint -sha256`. This is
#      the alignment that fixes the permanent admin HTTP 403.
#   2. The legacy SHA-1 fingerprint ($ssl_client_fingerprint) is a different,
#      40-char value, i.e. exactly why the old pipeline locked admins out.
...
if [ "$KEY_ALG" = "RSA" ]; then
    "$OSSL" req -x509 -newkey rsa:2048 -nodes -keyout "$WORK/c.key" -out "$WORK/c.crt" \
        -days 1 -subj "/C=BG/O=QS-CAP/OU=admin/CN=ops-admin" >/dev/null 2>&1
else
