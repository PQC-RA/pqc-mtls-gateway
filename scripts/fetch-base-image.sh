...
if [ "${PQC_SKIP_BASE_PULL:-0}" = "1" ]; then
  ...
  # Prefer the in-tree base.lock: a published snapshot carries it here and has
  # no deploy-locks branch to fetch. Upstream, where the branch is the source of
  # truth, the tree has no copy and the git fallbacks below are used.
