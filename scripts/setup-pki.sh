...
CNF
    ...
    "$OPENSSL" req -config "$INT_TLS_CNF" -new -x509 -days 3650 -extensions v3_ca \
        ...
        -subj "/C=BG/O=QS-CAP/OU=Internal Service Mesh/CN=PQC-GW Internal TLS CA" \
    ...
    "$OPENSSL" req -config "$INT_TLS_CNF" -new -key "$SSL_DIR/internal-tls.key" \
        -subj "/C=BG/O=QS-CAP/OU=Internal Service Mesh/CN=gateway" \
...
if [ -f "$PKI_DIR/root/certs/root-ca.crt" ]; then
    ...
    generate_internal_tls
    # The existing CA was never checked. A host bootstrapped before this
    # assertion, or with KEY_ALG overridden, is caught here rather than
...
sed "s#/etc/pki/pqc-ca#$PKI_DIR#g" "$TPL_DIR/openssl-root.cnf" > "$ROOT_CNF"
sed "s#/etc/pki/pqc-ca#$PKI_DIR#g; s#IP\.2  *=.*#IP.2  = $SERVER_IP#" \
...
"$OPENSSL" req -config "$INT_CNF" -new \
    ...
    -subj "/C=BG/O=QS-CAP/OU=PQC-Gateway/CN=pqc-gw.local" \
...
"$OPENSSL" req -config "$INT_CNF" -new \
    ...
    -subj "/C=BG/O=QS-CAP/OU=OCSP Responder/CN=ocsp.pqc-gw.local" \
